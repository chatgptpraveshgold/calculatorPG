...
export default async function handler(request, response) {
  ...
  // REAL-TIME: Ensure rates are fresh
  response.setHeader('Cache-Control', 's-maxage=1, stale-while-revalidate=59');

  // NEW SIMPLE NAME: Make sure you use "RATES_URL" in Vercel settings
  const gsheetUrl = process.env.RATES_URL;

  if (!gsheetUrl || gsheetUrl.length < 20) {
    return response.status(400).json({ error: 'RATES_URL not configured correctly in Vercel.' });
  }
  ...
  try {
    ...
    return response.status(500).json({ error: 'Real-time sync failed. Check your Google Script URL.' });
  }
}
